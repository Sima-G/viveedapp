jQuery(document).ready(function () {
    ...
    CKEDITOR.replace('session_description', {
        customConfig: "../../pages/modules/conference/sessions/ckeditor-config.js"
    });

    $('#sessions').html('<div class="text-center"><i class="fa fa-spinner fa-4x fa-spin"></i></div>');
    jQuery.ajax({
        url: "/backend/modules/conference/sessions/session_list",
        type: "GET",
    ...
    jQuery.ajax({
        url: "/backend/modules/conference/sessions/speaker_list",
        type: "GET",
        success: function (data) {
    ...
    $(document).on("click", '.session_edit', function (event) {
        ...
        if ($('#send-btn').length == 0) {
            $('#session_actions').append('<button type=\"submit\" id=\"send-btn\" class=\"btn btn-sm btn-primary send-btn\"><i class=\"fa fa-arrow-right\"></i> ' + send_btn_alt_txt + '</button>');
        }
    ...
    $(document).on("click", '.session_delete', function (event) {
        ...
        swal({
            ...
            function(){
                $.ajax({
                    url: '/backend/modules/conference/sessions/delete',
                    type: "post",
                    data: {
    ...
    submitHandler: function(form) {
        ...
        $.ajax({
            url: '/backend/modules/conference/sessions/store',
            type: "post",
            data: {
