...
class Cnf_speakerController extends Controller
{
    ...
    public function store()
    {
        ...
        if (Request::ajax()) {
            ...
            if(Input::has('speaker_action_id')){
                // find the bear
                $speaker = Cnf_speaker::find(Input::get('speaker_action_id'));
    ...
    public function show_speakers()
    {
        if (Sentinel::check()) {
            ...
            return View('backend.pages.modules.conference.speakers', compact('userRoles'));
        } else {
    ...
    public function delete()
    {
        if (Request::ajax()) {
            $data = Input::all();
            if(Input::has('speaker_action_id')) {
                Cnf_speaker::destroy(Input::get('speaker_action_id'));
                return Input::get('speaker_action_id');
            }
        }
    }
