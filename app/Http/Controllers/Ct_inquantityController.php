...
use Input;
use Sentinel;
use App\Ct_inquantity;

class Ct_inquantityController extends Controller
{
    ...
    public function inquantity_list($product_id)
    {
        // Getting all table data
        $inquantities = Ct_inquantity::where('product', '=', $product_id)->orderBy('id', 'ASC')->get();
        $userRoles = Sentinel::getRoles()->lists('name', 'id')->all();
        return View('backend.partials.modules.catering.inquantity_list', compact('userRoles', 'inquantities'));
    }
}
