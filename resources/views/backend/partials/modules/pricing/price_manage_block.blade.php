{{--{{ dd($master_product) }}--}}
{{--{{ dd($master_product->prc_catalogues->where('id', 2)->first()->pivot->price) }}--}}
{{--{{ dd($master_product->prc_catalogues->where('id', 2)->pivot->where('quantity', 2)->first()->price) }}--}}
<!-- Block Tabs -->
    ...
    <div class="tab-content">
        @foreach($catalogues as $key => $catalogue)
            <div class="tab-pane @if($key == 0) active @endif" id="catalogue_{{ $catalogue->id }}">
                ...
                @foreach($master_product->ctr_quantities as $product_quantity)
                    <div id="" class="row">
                        <form id="qnt_{{ $product_quantity->id }}_ctl_{{ $catalogue->id }}" action="" enctype="multipart/form-data" method="post" class="ctl_{{ $catalogue->id }} price_manage_form" onsubmit="return false;">
                            <div class="col-md-3">
                                <p class="form-control-static"><span id="quantity_{{ $product_quantity->id }}" class=""> {{ $product_quantity->pivot->quantity }} {{ $product_quantity->title }}</span></p>
                                <input type="hidden" name="product_action_id" value="{{ $master_product->id }}">
                                <input type="hidden" name="product_quantity_id" value="{{ $product_quantity->id }}">
                                <input type="hidden" name="product_catalogue_id" value="{{ $catalogue->id }}">
                                <input type="hidden" name="product_price_id" value="@if (!empty($prices->where('quantity', $product_quantity->id)->where('catalogue', $catalogue->id)->first()->id)){{ $prices->where('quantity', $product_quantity->id)->where('catalogue', $catalogue->id)->first()->id }}@endif">
                            ...
                            <div class="col-md-3">
                                <div class="input-group">
                                    {{--<input id="price_{{ $product_quantity->id }}" name="product_price" class="form-control" placeholder="@lang('backend/modules/pricing/products.price_help').." type="text">--}}

                                    <input id="price_{{ $product_quantity->id }}" name="product_price" class="form-control" placeholder="@lang('backend/modules/pricing/products.price_help').." type="text" value="@if (!empty($prices->where('quantity', $product_quantity->id)->where('catalogue', $catalogue->id)->first()->price)){{ $prices->where('quantity', $product_quantity->id)->where('catalogue', $catalogue->id)->first()->price }}@endif">
                                    <input type="hidden" id="old_price_{{ $product_quantity->id }}" name="old_price" value="@if (!empty($prices->where('quantity', $product_quantity->id)->where('catalogue', $catalogue->id)->first()->price)){{ $prices->where('quantity', $product_quantity->id)->where('catalogue', $catalogue->id)->first()->price }}@endif">
                            ...
                            <div class="col-md-3">
                                <div class="input-group">
                                    {{--<input id="discount_{{ $product_quantity->id }}" name="product_discount" class="form-control" placeholder="@lang('backend/modules/pricing/products.discount_help').." type="text" >--}}

                                    <input id="discount_{{ $product_quantity->id }}" name="product_discount" class="form-control" placeholder="@lang('backend/modules/pricing/products.discount_help').." type="text" value="@if (!empty($prices->where('quantity', $product_quantity->id)->where('catalogue', $catalogue->id)->first()->discount)){{ $prices->where('quantity', $product_quantity->id)->where('catalogue', $catalogue->id)->first()->discount }}@endif">
                                    <input type="hidden" id="old_discount_{{ $product_quantity->id }}" name="old_discount" value="@if (!empty($prices->where('quantity', $product_quantity->id)->where('catalogue', $catalogue->id)->first()->discount)){{ $prices->where('quantity', $product_quantity->id)->where('catalogue', $catalogue->id)->first()->discount }}@endif">

                                    {{--{{ dd($master_product->prc_catalogues) }}--}}
                                    {{--{{ dd($master_product->prc_catalogues->where('id', 2)->prc_product) }}--}}
                                    ...
                                    {{--<input type="hidden" id="old_discount_{{ $product_quantity->id }}" name="old_discount" value="@if (!empty($master_product->prc_products->pivot->where('quantity', $product_quantity->id)->where('catalogue', $catalogue->id)->first()->discount)){{ $master_product->prc_products->pivot->where('quantity', $product_quantity->id, 'catalogue', $catalogue->id)->first()->discount }}@endif">--}}
                                    <span class="input-group-addon"><i class="gi gi-star"></i></span>
                                </div>
                            </div>
