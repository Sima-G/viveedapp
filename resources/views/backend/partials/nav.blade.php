...
<ul class="sidebar-nav">
    ...
    <li @if((Route::current()->getName() == 'sessions') || (Route::current()->getName() == 'speakers') || (Route::current()->getName() == 'settings'))  class="active" @endif>
        ...
        <ul>
            {{--@if($settings->init != 0)--}}
                <li @if($settings->init == 0) class="content_not_for_init" @endif>
            ...
            <li>
                <a href="{{URL::route('settings')}}" @if(Route::current()->getName() == 'settings') class="active notice_init_tooltip" @endif @if($settings->init == 0) data-toggle="tooltip" data-original-title="@lang('master.msg_setting_init')" @endif ><span id="settings_nav_span" title="@lang('master.settings_desc')">@lang('master.settings')</span> @if($settings->init == 0) <i class="fa fa-exclamation-circle notice_init"></i> @endif</a>
            </li>
        </ul>
    </li>
</ul>
